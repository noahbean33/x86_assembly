...
strToInt:
            ...
            #RDI already contains string parameter for _strtol
            ...
            mov     eax, errno(%rip)     #Save in local variable
            pop     rax                  #Restore _strtol return value.
...
#
# If _strtol returns with endPtr == strToConv, then there were no
...
repeatPgm:  lea     fmtStr1(%rip), rdi
            ...
            call    _readLine
            lea     badInput(%rip), rbx   #Initialize state machine
            test    rax, rax        	  #RAX is -1 on bad input
            js      hadError        	  #(only neg value _readLine returns)
            
            #Call strtoint to convert string to an integer and
            #check for errors:
            
            lea     buffer(%rip), rdi     #Ptr to string to convert
            call    strToInt            
...
# input number.
            ...
            lea     goodInput(%rip), rbx
            mov     eax, inputValue(%rip)
...
# Transfer here if _readLine returned an error:

badInput:   lea     badInpStr(%rip), rdi
            mov     $0, al
            call    _printf
            jmp     repeatPgm
...
invalid:    lea     invalidStr(%rip), rdi
            mov     $0, al
            call    _printf
            jmp     repeatPgm
...
unknown:    lea     unknownStr(%rip), rdi
            mov     $0, al
            call    _printf
            jmp     repeatPgm
...
goodInput:  lea     goodStr(%rip), rdi
            mov     inputValue(%rip), esi #Zero extends!
            mov     $0, al
            call    _printf
            jmp     repeatPgm
